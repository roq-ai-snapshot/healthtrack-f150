...
import { getCampaignPerformanceById, updateCampaignPerformanceById } from 'apiSdk/campaign-performances';
...
function CampaignPerformanceEditPage() {
  ...
  const handleSubmit = async (values: CampaignPerformanceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateCampaignPerformanceById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
